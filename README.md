# Lista de Exercícios de Recursividade

### Status da Lista: Concluída ✔️ <br> <br>

## 📝Descrição da Lista

Este repositório contém uma coleção de 30 exercícios de recursividade desenvolvidos como parte da disciplina de Estrutura de Dados I, ministrada pelo Prof. Mayck Diego Cipriano. O material foi elaborado com foco no treinamento prático e na consolidação dos conceitos de recursividade. <br>

Os exercícios abrangem desde problemas clássicos (como Fatorial, Fibonacci) até algoritmos mais complexos (como Backtracking, N-Rainhas, Ordenação e Busca), permitindo uma progressão de dificuldade. <br> <br>

## 🎯 Objetivos de Aprendizagem

*	Desenvolver o raciocínio recursivo 
*	Implementar algoritmos clássicos 
*	Compreender a complexidade de soluções recursivas 
*	Aplicar a recursividade em problemas reais.

## 💻 Tecnologias Utilizadas

![C](https://img.shields.io/badge/C-0?style=for-the-badge&logo=c&logoColor=%23ffffff&labelColor=%23AABACC&color=%23AABACC) <br> <br>

## ⚡ Exercícios

### 🔹 Nível Básico

1.	Fatorial 
2.	Soma de Números Naturais 
3.	Sequência de Fibonacci 
4.	Potência 
5.	Contagem Regressiva 
6.	Soma de Dígitos 
7.	Inversão de String 
8.	Verificação de Palíndromo <br>
    
### 🔹 Nível Intermediário

9.	Máximo Divisor Comum (MDC) 
10.	Multiplicação Recursiva 
11.	Contagem de Caracteres 
12.	Soma de Elementos de um Array 
13.	Verificação de Número Primo 
14.	Conversor Decimal → Binário 
15.	Números Pares 
16.	Números Ímpares 
17.	Busca em Array 
18.	Comprimento de String 
19.	Torre de Hanói <br>
    
### 🔹 Nível Avançado

20.	Permutação de String 
21.	Permutação de Array 
22.	Subconjuntos de um Array 
23.	Merge Sort 
24.	Quick Sort 
25.	Busca Binária 
26.	Exponenciação Rápida 
27.	Caminho em Matriz 
28.	Problema das N-Rainhas 
29.	Labirinto com Backtracking 
30.	Gerador de Parênteses Válidos <br>
    
Nota: Alguns exercícios mais avançados foram desenvolvidos com apoio de ferramentas de IA para validação e compreensão dos algoritmos. <br> <br>

## 📂 Estrutura

```
/exercicios
├── basico/
├── intermediario/
└── avancado/
```

## ✨Autor

<img src = "https://avatars.githubusercontent.com/u/187053289?v=4" width = 150px>
<strong> Victor </strong>
<br><br>
<a href = "https://www.linkedin.com/in/victor-m-rodrigues/"><img alt="Static Badge" src="https://img.shields.io/badge/LinkedIn-0?style=for-the-badge&logoColor=%23FFFFFF&color=%230077B5&link=https%3A%2F%2Fwww.linkedin.com%2Fin%2Fvictor-m-rodrigues%2F"></a> 
<a href = "https://github.com/Victor-M-S-Rodrigues07/"><img alt="Static Badge" src="https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23ffffff&labelColor=%23000000&color=%2300000000"></a>
